  ...
  return (
    <div className='text-white w-80 flex-grow h-3/4 flex group hover:justify-center hover:items-center  shadow border rounded-xl flex-shrink-0  relative overflow-hidden'>
       ...
       <h3 className='text-sm text-[#D4F6FF] font-sans font-semibold'>{character.role}</h3>
        <h1 className='text-3xl font-black font-serif flex w-full items-center gap-5 text-[#f2f2f2]'>{character.name} <span className='text-sm font-semibold font-sans'> {character.age}</span></h1>
        <h2 className='font-shadows scrollbar text-base h-20 opacity-60 font-semibold overflow-y-scroll w-full'>{character.famousLine}</h2>
       </div>
       </div>
    </div>
  )
